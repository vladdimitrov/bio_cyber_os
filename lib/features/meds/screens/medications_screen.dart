...
class _MedicationsScreenState extends State<MedicationsScreen>
  ...
  Future<void> _insertMedicationLog(
    ...
    try {
      final base = _takenAtLocalForNewLog();
      ...
      if (picked.reminder.enabled) {
        if (picked.reminder.mode == ReminderMode.atConsumptionTime) {
          reminderLocal = scheduledAt;
          reminderOffsetMinutes = 0;
        } else {
          final off = picked.reminder.offsetMinutes;
          reminderLocal = scheduledAt.subtract(Duration(minutes: off));
      ...
      for (var i = 0; i < n; i++) {
        ...
        final scheduledI = scheduledAt.add(dayShift);
      ...
      for (var i = 0; i < insertedList.length; i++) {
        ...
        final schedLocal = scheduledAt.add(Duration(days: i));
...
class _LogMedIntakeDialog extends StatefulWidget {
  ...
  const _LogMedIntakeDialog({required this.medicationName});
...
class _LogMedIntakeDialogState extends State<_LogMedIntakeDialog> {
  ...
  late String _unit;
  TimeOfDay _intakeTime = TimeOfDay.now();
